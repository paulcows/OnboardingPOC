...
app.post(properties.api + '/changePassword', function(req, res) {
    ...
    db_OnboardingPOC_db.User.findOne({ username: req.user.username, password: req.body.passwordOld }).exec(function(err, user) {
...
var getUser = function(username, password, callback) {
    ...
    db_OnboardingPOC_db.User.findOne({ username: username, password: password }).lean().exec(function(err, user) {
        callback(user);
    });
};
...
var createUser = function() {
    ...
    db_OnboardingPOC_db = require('../db/OnboardingPOC_db_schema.js');
    db_OnboardingPOC_db.User.count().exec(function(err, count) {
        if (count == 0) {
            var admin = new db_OnboardingPOC_db.User({
                username: 'admin',
